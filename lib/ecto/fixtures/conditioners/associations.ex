defmodule EctoFixtures.Conditioners.Associations do
  ...
  defp has_through_association(data, [source, table_name, :rows, _row_name] = path, %{cardinality: :one} = association) do
    ...
    { data, inverse_association_path } = get_path(data, path, get_in(data, path ++ [:data, field]))
  ...
  defp get_path(data, path, {{:., _, [{{:., _, [{:fixtures, _, [file_path]}, inverse_table_name]}, _, _}, inverse_row_name]}, _, _}) do
    ...
    inverse_source_data = EctoFixtures.read(inverse_source)
    |> EctoFixtures.parse
